...
| `/ingest [source]` | Ingest | `wiki-ingest` skill; accepts a `source_id`, a path/URL, or an attachment/pasted text block (auto-landed via `wiki-add`); no argument ingests the next worklist item |
