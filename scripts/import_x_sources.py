...
Usage:
  python3 scripts/import_x_sources.py --archive ~/Downloads/twitter-2026-06-24-abc123
  python3 scripts/import_x_sources.py --bookmarks-json ~/Downloads/bookmarks.json
  python3 scripts/import_x_sources.py --clips "~/path/to/AI Ideas /Tweets"
  python3 scripts/import_x_sources.py --archive PATH --bookmarks-json PATH --months 12
...
def parse_archive_likes(archive_dir: Path) -> list[dict]:
    ...
    *,
    ...
    lines.extend(
        [
            ...
            "---",
            "",
            text.strip() or "_Tweet text not included in export; ID preserved for later enrichment._",
            "",
        ]
    )
    return "\n".join(lines)
...
def import_xarchive_bookmarks(json_path: Path, months: int, dry_run: bool) -> tuple[int, int]:
    ...
    items = payload if isinstance(payload, list) else payload.get("bookmarks") or payload.get("tweets") or []
    for item in items:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--months", type=int, default=12, help="Only import items from last N months")
